import { type LogFileEntry, type LogRead, parseLogRead, parseLogsList } from "./wire";
...
export function useLogs() {
    ...
    const files = useQuery({
        ...
        queryFn: async () => parseLogsList(await api.sandbox.json(`/logs`)),
...
export function useLogTail(name: Ref<string | undefined>, bytes: Ref<number>) {
    ...
    const tail = useQuery({
        ...
        queryFn: async () =>
            parseLogRead(await api.sandbox.json(`/logs/file?name=${encodeURIComponent(name.value ?? ``)}&bytes=${bytes.value}`)),
